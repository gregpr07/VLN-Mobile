...
class AuthorSerializer(serializers.ModelSerializer):
    def to_representation(self, instance):
        ...
        serialized_data["categories"] = SimpleCategorySerializer(
            instance.get_categories(), many=True
        ).data
...
class CategorySerializer(serializers.ModelSerializer):
    def to_representation(self, instance):
        ...
        serialized_data["children"] = SimpleCategorySerializer(
            instance.get_children(), many=True, read_only=True
        ).data
        serialized_data["authors"] = SimpleAuthorSerializer(
            instance.get_authors(),
            many=True,
...
class EventSerializer(serializers.ModelSerializer):
    def to_representation(self, instance):
        ...
        serialized_data["lectures"] = SimpleLectureSerializer(
            instance.get_lectures(), many=True
        ).data

        categories = []
        for category in instance.get_categories():
            categories.append(
                SimpleCategorySerializer(Category.objects.get(id=category)).data
            )
    ...
    def to_representation(self, instance):
        ...
        if user.is_authenticated:
            ...
            last_view = LectureView.objects.filter(
                user=request.user, lecture=instance
            ).first()
            serialized_data["left_off"] = (
                0 if last_view is None else last_view.end_timestamp
            )
...
class SimpleLectureSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Lecture
        fields = ("id", "title", "thumbnail", "author", "views", "event", "published")
